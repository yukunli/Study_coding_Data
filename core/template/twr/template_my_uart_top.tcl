...
if {[is_project_open]} {
	if {[string compare $quartus(project) "my_uart_top"]} {
		puts "Project my_uart_top is not open"
		set make_assignments 0
	}
} else {
	# Only open if not already open
	if {[project_exists my_uart_top]} {
		project_open -revision my_uart_top my_uart_top
	} else {
		project_new -revision my_uart_top my_uart_top
	}
